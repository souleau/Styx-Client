...
Kisse::Module::Module(const char* module_name) :
	Module(GetModuleHandleA(module_name))
{
}

Kisse::Module::Module(HMODULE a_module) :
	module(a_module)
{
	GetModuleInformation(GetCurrentProcess(), module, &moduleInfo, sizeof(MODULEINFO));
}

uint8_t* Kisse::Module::pattern_scan(uint8_t pattern[], int size, int access) const
{
...
std::vector<uint8_t*> Kisse::Module::pattern_scan_all(uint8_t pattern[], int size, int access) const
{
...
uint8_t* Kisse::Module::pattern_scan_text_section(uint8_t* const pattern_start, const int pattern_size, const uint8_t wildcard) const
{
...
void* Kisse::Module::getProcAddress(const char* name)
{
	return GetProcAddress(module, name);
}

Kisse::Module::operator bool() const
{
	return module && moduleInfo.lpBaseOfDll;
}

uint8_t* Kisse::Module::allocate_nearby_memory(uint8_t* nearby_addr, int size, int access)
{
	for (int i = 65536;
		i < 0x7FFFFFFF;
		i += 65536)
...
PIMAGE_NT_HEADERS Kisse::Module::get_nt_headers() const
{
	PIMAGE_DOS_HEADER dos_header = (PIMAGE_DOS_HEADER)module;
	return (PIMAGE_NT_HEADERS)((uint8_t*)module + dos_header->e_lfanew);
}

std::vector<PIMAGE_SECTION_HEADER> Kisse::Module::get_section_headers() const
{
