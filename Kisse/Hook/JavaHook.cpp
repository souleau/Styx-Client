...
bool Kisse::JavaHook::hook(jmethodID methodID, i2i_detour_t detour)
{
    static int runonce = []()->int
    {
            jvmtiCapabilities capabilities{ .can_retransform_classes = JVMTI_ENABLE };
            Kisse::p_tienv->AddCapabilities(&capabilities);
            return 0;
    }();
    if (!methodID || !detour) 
        return false;
    ...
    *flags |= (NO_COMPILE);
    ...
    Kisse::p_tienv->GetMethodDeclaringClass(methodID, &owner);
    Kisse::p_tienv->RetransformClasses(1, &owner); //small trick to delete any already compiled / inlined code
    Kisse::p_env->DeleteLocalRef(owner);
...
Kisse::JavaHook::Midi2iHook::Midi2iHook(uint8_t* target, i2i_detour_t detour) :
    target(target)
{
...
Kisse::JavaHook::Midi2iHook::~Midi2iHook()
{
    if (is_error)
        return;
