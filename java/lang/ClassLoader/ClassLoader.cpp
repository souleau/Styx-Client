...
ClassLoader ClassLoader::getMinecraftClassLoader(JNIEnv* env)
{
	...
	Kisse::p_tienv->GetAllThreads(&thread_count, &thread_list);
	for (jint i = 0; i < thread_count; ++i)
	{
		jvmtiThreadInfo thread_info{};
		Kisse::p_tienv->GetThreadInfo(thread_list[i], &thread_info);
		if (thread_info.name && !std::strcmp(thread_info.name, "Client thread"))
		{
			classLoader = thread_info.context_class_loader;
			Kisse::p_tienv->Deallocate((unsigned char*)thread_info.name);
	...
	Kisse::p_tienv->Deallocate((unsigned char*)thread_list);
